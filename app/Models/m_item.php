...
class m_item extends Model
{
    ...
    public function get_barang(string $id_umkm, string $keyword = null)
    {
        ...
        if ($keyword === null)
        {
            $sql = "SELECT nama_barang, harga_barang, stok_barang FROM barang WHERE id_umkm = '$id_umkm'";
        }
        else
        {
            $sql = "SELECT nama_barang, harga_barang, stok_barang FROM barang WHERE id_umkm = '$id_umkm' AND nama_barang LIKE '%$keyword%'";
        }
